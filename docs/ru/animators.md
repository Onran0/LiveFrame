...
- - `name` - имя слоя;
- - `*mask` - имя маски, которая используется в слое. по-умолчанию используются все маски;
- - `*masks` - массив с именами масок, которые используются в слое. не должен определяться вместе с `mask`;
